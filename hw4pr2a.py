"""
Start file for hw4pr2 part(a) of Big Data Spring 2019
...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	sigma = sigmoid(X @ W)
	nllFunction = np.multiply(y, np.log(sigma)) + np.multiply((1.-y), np.log(1. - sigma))
	nll = -sum(nllFunction) + reg/2 * np.linalg.norm(W) ** 2
	"*** END YOUR CODE HERE ***"
	return nll.item(0)
...
def newton_step(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = sigmoid(X @ W)
	grad = grad_logreg(X, y, W, reg=0.0)
...
def newton_method(X, y, reg=0.0, eps=1e-6, max_iter=20, print_freq=5):
	...
	while iter_num < max_iter and np.linalg.norm(step) > eps:
		...
		if np.isnan(nll):
			break
		nll_list.append(nll)
		newtonStep = newton_step(X, y, W, reg = reg)
		W -= newtonStep
		"*** END YOUR CODE HERE ***"
...
def get_description(X, y, W):
	"""	This function takes in three arguments:
		...
		HINT:
			1) Get the predict labels using predict defined above
